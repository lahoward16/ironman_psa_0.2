...
formatted$psa6mo <- all_psa %>%
  ...
  mutate(psa_nadir6gp = cut(psa, 
                            breaks = c(-1, 0.02, 0.2, 1000000), 
                            labels = c("<0.02", "0.02-0.19", "≥0.2"),
                            right = FALSE)) %>% 
  select(subject, psa_nadir6 = psa, psa_nadir6gp, psa_nadir6_date = psa_date)


  # keep PSAs within 6 mo after treatment start
test <- psa_for_merge %>% left_join(formatted$treatment_pivot, by="subject") %>%
  mutate(timetopsa = time_length(psadt_int - dt_lot_start, unit = "month")) %>%
  filter(timetopsa>0 & timetopsa<=6) %>%
  # # keep PSAs before second treatment regimen
  left_join(formatted$tmt_change, by="subject") %>%
  filter(psadt_int<treatment_change_date | is.na(treatment_change_date)) %>%
  # select lowest PSA
  arrange(subject, psa, psadt_int) %>% 
  group_by(subject) %>% 
  slice(1) %>% 
  # categorize PSA
  mutate(psa_nadir6gp = cut(psa, 
                            ...
                            labels = c("<0.02", "0.02-0.19", "≥0.2"),
                            right = FALSE)) %>% 
  select(subject, psa_nadir6 = psa, psa_nadir6gp, psa_nadir6_date = psadt_int)
...
all_data2 <- all_data %>%
  mutate(
    ...
    limbo6 = ifelse(dead==1, time_length(deathdate_int - psa_nadir6_date, unit = "month"), 
                   time_length(new_censor_date - psa_nadir6_date, unit = "month")),
    limbo12 = ifelse(dead==1, time_length(deathdate_int - psa_nadir12_date, unit = "month"), 
                   time_length(new_censor_date - psa_nadir12_date, unit = "month")),
   
    # diagnosis to treatment start
    diag_tmt_start = time_length(dt_lot_start1 - diagbx_date_int, unit = "month"),
    alk150 = ifelse(alk>150, 1, 0),
    pre_tmt_psa20 = ifelse(pre_tmt_psa>20, 1, 0)
  )
...
all_data2 <- all_data2 %>%
  mutate(
    ...
    alk150 = factor(alk150, levels = 0:1, labels = c("≤150", ">150")),
    pre_tmt_psa20 = factor(pre_tmt_psa20, levels = 0:1, labels = c("≤20", ">20")))
